...
;(function($) {	
	$.iPhorm = function ($form, options) {
		...
		_this.applyElementLogic = function (elementId, loading) {
			if (window.iphormLogic && window.iphormLogic[settings.id] && window.iphormLogic[settings.id].logic) {
				var logic = window.iphormLogic[settings.id].logic[elementId];
				...
				if (logic && logic.rules && logic.rules.length) {					
					...
					if (!loading && window.iphormLogic[settings.id].animate) {
						if (action == 'show') {
							$element.slideDown();
						} else {
							$element.slideUp();
		...
		_this.applyDependentLogic = function (elementIds) {
			if (window.iphormLogic && window.iphormLogic[settings.id] && window.iphormLogic[settings.id].dependents) {
				for (var i = 0; i < elementIds.length; i++) {
					var dependentElementIds = window.iphormLogic[settings.id].dependents[elementIds[i]],
	...
	window.iPhorm = {
		...
		preload: function (images, prefix) {
			for (var i = 0; i < images.length; i++) {
				...
				window.iPhorm.preloadedImages.push(elem);
			}
		}
	};
